...
export function createDefaultObject(schema, obj = {}){
...
export function getMultipleFields(schema) {
	let res = [];
	each(schema.fields, (field) => {
		if (field.multi === true) 
			res.push(field);
...
export function mergeMultiObjectFields(schema, objs) {
	let model = {};
...
export function slugifyFormID(schema, prefix = "") {
